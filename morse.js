...
NodeList.prototype.classify = function() {
    ...
    if (this.numUnlabeled > 0) {
        for(var i in this.unknownNode) {
            ...
            for(var j = 0; j < this.searchIndex; j++) { //compare closest neighbors, regardless of state

                if(this.currentUnknown.state == this.closest[j].node.state) { //specify state
...
NodeList.prototype.constructWord = function() {
    ...
    //console.log("signal array i: " + this.signalArray); 

    //console.log("morse dictionary i: " + morseDictionary['I']);
    for (var j in morseDictionary) {
        ...
        if (morseDictionary[j].length == this.signalArray.length) {
            ...
            //console.log("entered");
            //console.log(morseDictionary[j]);

            for (var k in morseDictionary[j]) {
                //console.log("morse dictionary: " + morseDictionary[j][k]);
                //console.log("signal array: " + this.signalArray[k]);
...
NodeList.prototype.labelCategory = function() {
    ...
    //console.log(findMajority);
    //console.log(this.currentUnknown);
}
